...
def myProfile(request,id):
    ...
    profile_photo = profile_photo.objects.get(user = user) 
...
def view_users(request, user_id):
    profile_photo= Profile.objects.filter(id=user_id).all()
    my_photos = Image.objects.filter(profile_id=user_id)
    users=User.objects.filter(id=user_id).all()

    return render(request, "other.html", {"photos":my_photos, "profile":profile_photo, "users":users})
